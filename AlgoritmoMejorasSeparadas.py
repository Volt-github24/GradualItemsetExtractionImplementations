...
"""Función con la mejora de matrices comprimidas"""
...
"""Función alternativa con la mejora de precisión"""
def generate_matrix_precision(df, r, g_item1, g_item2):
    at1 = pd.to_numeric(df[g_item1[0]])
    op1 = g_item1[1]
    ...
    N = len(at1)
    matrix = np.zeros((N, N), dtype = np.uint) # Length is the number of transactions...
...
"""Función alternativa con la mejora de precisión"""
def compute_support_precision(m):
    N = m.shape[1]
    return np.sum(m/255)/(N*((N-1)/2))
            ...
            for j in range(i+1,num_candidates):
                ...
                if (current_candidate != 0):
                    ...
                    if(already_generated == 0):
                        ...
                        candidate_matrix = csr.minimum(matrices[i],matrices[j]) 
                        candidate_support = compute_support(candidate_matrix)
                        if(candidate_support >= min_supp):
                            frequent_matrices += [candidate_matrix]
...
"""Función alternativa con la mejora de precisión"""
def extractFrequentItemsets_precision(dataset, min_supp, r):
    ...
    frequent_candidates = []
    for itemset in candidates:
        candidate_matrix = generate_matrix_precision(dataset, r, itemset[0], itemset[1])
        candidate_support = compute_support_precision(candidate_matrix)
        if(candidate_support >= min_supp):
            frequent_matrices += [candidate_matrix]
            frequent_candidates += [itemset]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mejora', action='store', dest='mejora', default = 'm2', choices = ['m1','m2'],
                    help='Mejora usada: m1 -> matrices comprimidas de Yale. m2 -> mejora de precision con 8 bits')
    ...
    if(mejora == 'm1'):
        ...
        result = extractFrequentItemsets_precision(normalized_data, min_supp,r)
    ...
    print("Tiempo de ejecucion del algoritmo con una de las mejoras: ", t2-t1)
