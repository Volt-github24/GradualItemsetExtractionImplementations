...
def generate_matrix(df, r, g_item1, g_item2):
    ...
    N = len(at1)
    matrix = np.zeros((N, N), dtype = np.float16) # Length is the number of transactions...
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_trans','-tr', action='store', dest='num_transaciones', default = 50,
                    help='Subconjunto de transacciones que analizar de la base de datos')
    
    parser.add_argument('--soporte','-s', action='store', dest='soporte_minimo', default = 0.6,
    ...
    print("Tiempo de ejecucion del algoritmo mejorado: ", t2-t1)
